...
rsync -avzP --delete -L -K -e "ssh -p ${SERVER_PORT}" portal-tasks "${SERVER_USER_AT_IP}:${SERVER_REPORTS_PATH}"

echo "=> Pushing tasks to the container"
ssh -p ${SERVER_PORT} ${SERVER_USER_AT_IP} "rails_container=\$(docker ps --format '{{.Names}}' | grep ${SERVER_CONTAINER_BASENAME}) && docker cp ${SERVER_REPORTS_PATH}/portal-tasks \$rails_container:/rails"
